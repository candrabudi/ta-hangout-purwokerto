...
class HomeController extends Controller
{
    ...
    public function show($slug, Request $request)
    {
        ...
        $recommendedBasedOnRating = Hangout::selectRaw('hangouts.*, AVG(visitor_interactions.rating_value) as avg_rating')
            ...
            ->where('interaction_type', 'rating')
            ->groupBy('hangouts.id', 'location_id', 'hangouts.name', 'hangouts.slug', 'hangouts.address', 'hangouts.description', 'hangouts.thumbnail', 'hangouts.status', 'hangouts.google_maps_url', 'hangouts.longtitud', 'hangouts.latitud')
            ->having('avg_rating', '>=', 3)
            ->inRandomOrder()
            ->take(5)
            ->get();
